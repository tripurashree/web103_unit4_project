...
const componentPrices = {
  color: { Red: 600, Blue: 500, Black: 450, White: 400 },
  engine: { 'V6': 4000, 'V8': 3000, 'Electric': 2000 },
  interior: { Leather: 1500, Fabric: 1000 },
...
const EditCar = ({ title }) => {
  ...
  useEffect(() => {
    const fetchCar = async () => {
      ...
      setName(car.name);
      setOptions(car.features); // Adjusted to match features structure
    };
    fetchCar();
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const updatedCar = { name, features: options, total_price: price }; // Matching updated structure
